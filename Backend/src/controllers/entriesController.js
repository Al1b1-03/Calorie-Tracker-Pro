...
export const getStats = async (req, res) => {
  try {
    ...
    const remaining = calorieNorm - todayCalories;
...
export const getChart = async (req, res) => {
  try {
    ...
    const dateFrom = toStr(start);
    const dateTo = toStr(end);

    const result = await query(
      `SELECT entry_date, COALESCE(SUM(calories), 0) AS calories
       FROM food_entries
    ...
    while (cursor <= endDate) {
      byDate[toStr(cursor)] = 0;
      cursor.setDate(cursor.getDate() + 1);
    }

    result.rows.forEach((row) => {
      const key = String(row.entry_date).slice(0, 10);
      byDate[key] = parseInt(row.calories, 10) || 0;
    });

    const data = Object.entries(byDate).map(([date, calories]) => ({ date, calories }));
